...
class DesktopPet(QWidget):
    ...
    def __init__(self):
        ...
        self._pet_pixmap = get_pet_pixmap()  # 原图只加载一次, 缩放时复用
    ...
    def _init_pet(self):
        ...
        if not self._pet_pixmap.isNull():
            scaled = scale_pixmap(self._pet_pixmap, w, h)
    ...
    def _init_animations(self):
        ...
        # 空闲浮动: 记录已流逝时间, 用正弦波驱动 (周期 = ANIM_FLOAT_SPEED)
        self._float_elapsed = 0
    ...
    def _float_update(self):
        ...
        self._float_elapsed += self.float_timer.interval()
        phase = (self._float_elapsed / config.ANIM_FLOAT_SPEED) * 2 * math.pi
        y_offset = int(math.sin(phase) * config.ANIM_FLOAT_AMPLITUDE * self.scale)
    ...
    def _open_toolbox(self):
        """打开/关闭工具箱 (切换式, 保证同一时间只有一个实例)"""
        # 若已有工具箱 (无论处于何种状态), 一律关闭旧实例, 防止叠加多个窗口
        if self.toolbox is not None:
            old = self.toolbox
            self.toolbox = None
            # 断开信号, 避免关闭回调再改写 self.toolbox
            try:
                old.closed.disconnect(self._on_toolbox_closed)
            except (TypeError, RuntimeError):
                pass
            old.close_animated()  # 淡出关闭; 配合 WA_DeleteOnClose 自动销毁
            return
    ...
    def mouseReleaseEvent(self, event):
        if event.button() == Qt.LeftButton:
            if not self._is_dragging:
                ...
                self._show_bubble(random.choice(BUBBLE_TEXTS))
                self._open_toolbox()
