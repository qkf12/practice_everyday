...
class NotepadTool(BaseTool):
    ...
    def get_widget(self, parent=None):
        ...
        self._msg_timer = None       # 提示消息的自动隐藏定时器
    ...
    def _update_char_count(self):
        ...
        if len(text) > config.NOTEPAD_MAX_CHARS:
            # 截断到上限, 保持光标位置
            cursor_pos = self.text_edit.textCursor().position()
            self.text_edit.blockSignals(True)
            self.text_edit.setPlainText(text[:config.NOTEPAD_MAX_CHARS])
            cursor = self.text_edit.textCursor()
            cursor.setPosition(min(cursor_pos, config.NOTEPAD_MAX_CHARS))
            self.text_edit.setTextCursor(cursor)
            self.text_edit.blockSignals(False)

        count = len(self.text_edit.toPlainText())
        over_limit = count >= config.NOTEPAD_MAX_CHARS
        color = config.COLOR_ACCENT_RED if over_limit else config.COLOR_TEXT_SECONDARY
        suffix = " (已达上限)" if over_limit else ""
        self.char_label.setStyleSheet(f"""
            QLabel {{
                color: {color};
                font-size: 12px;
        ...
        """)
        self.char_label.setText(f"{count} 字{suffix}")
    ...
    def _show_message(self, text, is_error=False):
        ...
        # 复用同一个定时器, 避免旧消息的计时器提前清掉新消息
        if self._msg_timer is None:
            self._msg_timer = QTimer(self)
            self._msg_timer.setSingleShot(True)
            self._msg_timer.timeout.connect(lambda: self.success_label.setText(""))
